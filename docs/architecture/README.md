...
| Replication | Native Keylane replication, Redis PSYNC interoperability, destructive full sync, online logs, Sentinel role changes, source-history retention, fenced promotion, replay, and the fail-closed single-group cluster boundary | [Replication](05-replication.md) |
| Cluster data plane | Slot routing, finite authority admission, node-local control installation, Meta discovery/session handling, controlled-failover activation, and Redis Cluster compatibility | [Cluster data plane](06-cluster-data-plane.md) |
| Meta control plane | NuRaft-backed metadata, static multi-member genesis, Data-session publishing, leader-local observations, administration, cluster status, leader-owned recovery of creation, post-genesis membership, and controlled-failover workflows, durable failover recovery handoff, membership identity, and WAL/snapshots | [Meta control plane](08-meta-control-plane.md) |
...
| Replication has manager, boot-scoped single-group coordination, callable cluster-rebuild, source-hold, frozen-source, and prepared-activation adapters behind fail-closed admission, Sentinel-compatible configuration, and storage-log integration boundaries | `include/keylane/replication.h`, `include/keylane/replication_group.h`, `src/config.cpp`, `src/replication/`, `src/storage/engine/replication_log.cpp`, `tests/replication_group_test.cpp`, `tests/cluster/population_integration_test.cpp`, `tests/cluster/rebuild_protocol_integration_test.cpp`, `tests/cluster/source_history_hold_integration_test.cpp` |
| Cluster data plane has topology, authority, node-controller, static-control, Meta-client/session, controlled-failover adaptation, and Redis gate boundaries | `include/keylane/cluster/`, `src/cluster/`, `src/redis/cluster_command.cpp`, `src/redis/command.cpp`, `src/redis/server.cpp` |
| Meta control plane separates deterministic committed state (including group failover recovery), pure node projection, volatile observations, leader-scoped Data publishing and workflow reconciliation, authenticated administration, static initial membership, stable cluster status and initial creation, and NuRaft integration | `include/keylane/meta/`, `src/meta/`, `app/keylane_meta.cpp`, `app/keylane_ctl.cpp` |
